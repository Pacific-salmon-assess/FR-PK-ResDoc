...
This diversion rate appears to be increasing (i.e., increasingly dominated by northern migration route), which could have implications for survival **add why**, and can affect accuracy of forecasts [@hagueMovingTargetsAssessing2021].  
...
Catch has typically been estimated by multiplying the **total coastal (check)** Pink catch (Canada and US) by the estimated contribution of Fraser stocks to the coastal catch. 
...
These measures have included: time and area closures (e.g., Interior Fraser Coho fishing window closure), gear requirements (e.g., use of beach seines or shallow seines instead of drift gill nets, bait ban for recreational fisheries), and operational changes (e.g., brailing requirements and maximum recommended set sizes for purse seines) (*cite - AMH*). 
...
In the last five generations (10 years, 5 returns), the run size and escapement have been stable, with a slight increase, with an average run size of **check code** `r mean(filter(data, year >=2015)$harvest + filter(data, year >=2015)$spawn`7.4 million, average escapement of `r mean(filter(data, year >=2015)$spawn)`6.9 million, and an average ER of `r round(mean(filter(data, year >=2015)$harvest/ (filter(data, year >=2015)$harvest + filter(data, year >=2015)$spawn))*100, 0)` (Figure \@ref(fig:fig-catch-esc-HCR)).  
...
An increased interest in Pink harvest [@macdonaldStateCanadianPacific2023], paired with the revised *Fisheries Act* [@DFO1985Act] make it necessary to carefully re-examine this population's dynamics and current management.  
