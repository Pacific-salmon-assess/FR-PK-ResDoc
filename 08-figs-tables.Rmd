...
csasdown::csas_table(priors,
    ...
    kableExtra::column_spec(4, width = "2cm") #doesn't get smaller - wtf
}

if(FALSE){
    priors2 <- read.csv(here("analysis/data/raw/Table_priors2.csv"))
   ...
   mutate_all(function(x){gsub("\\\\n","\n", x)}) |>
    ...
    #\\\\sim N(ln(1/S_{MAX}) - 0.5\\\\sigma_{S_{MAX}}^2, \\\\sqrt{ln(1+ \\\\cfrac{(\\\\cfrac{1}{\\\\sigma S_{MAX}})^2}{(\\\\cfrac{1}{S_{MAX}})^2}})) #to debug Smax prior
}
if(TRUE){
#ugh write it all out as above?
priors.df <- data.frame(#Parameter = c("$ln(\\alpha)$", 
                        ...
                        #              "$\\sigma_{R_0}$"), 
                        Prior = c("$\\sim N(1,2)$", 
                                  "$\\sim N\\bigg(ln(1/S_{MAX}) - 0.5\\sigma_{S_{MAX}}^2, \\sqrt{ln(1+ \\cfrac{(\\cfrac{1}{\\sigma S_{MAX}})^2}{(\\cfrac{1}{S_{MAX}})^2}})\\biggr)$", 
                                  "$\\sim U(-1,1)$", 
                                  "$\\sim N(1,2)$", 
                                  "$\\sim N(0,20)$", 
                                  "$\\sim Inv-Gamma(2,1)$"))
                        ...
                        #          "$[0,\\inf]$",
                        #          "[0,\\inf]$", 
                        #          "$[0,\\inf]$")#, turned off to isolate problem
...
priors.df |>
  ...
  csasdown::csas_table(format = "latex",
                     caption = "(ref:tab-priors)", 
                     repeat_header = TRUE)# |>
    #kableExtra::column_spec(2, width = "5cm") #off to isolate problem
}
