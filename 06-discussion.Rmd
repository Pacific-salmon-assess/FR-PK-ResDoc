...
The time-varying productivity model used to condition the forward simulation showed that productivity of Fraser Pinks has been declining since the 1980s, with current productivity nearly half of what it was at it's peak in the 1980s. 
This, paired with the recent negative residuals in the static model, highlights how low the recent productivity is.  
...
The robustness test of the current HCR suggest that if stock productivity were to decline sharply and/or be depressed for a prolonged period the current HCR would have ~5% more in a "red" zone and  ~14% more in "amber" zone, with ~7m less fish caught. 
In the robustness test the alternative HCR performed relatively worse than the current HCR, possibly due to the higher allowable exploitation rates between this HCRs operational control points. 
To mitigate these biological risks, an option could be to use a dynamic version of our alternate HCR, where the removal reference $U_{MSY}$ changes in relation to current estimates of time-varying productivity.  
