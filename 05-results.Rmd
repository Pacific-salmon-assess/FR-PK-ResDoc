...
The spawner-recruit model successfully converged with a maximum R-hat of 1.003, traceplots that suggested MCMC chains were well mixed, and a minimum ESS of `r round(min(model.summary$n_eff, na.rm = TRUE), 0)`, and chains that appeared well mixed (See Supplement *B - make ref* for fully reproducible analysis and full diagnostics). 

The spawner-recruit relationship showed evidence of compensation at low spawner abundances (below ~5M), overcompensation at high spawner abundances (over ~10M) and a relatively poor fit between 4-10 million (Figure \@ref(fig:fig-SRR)).  


## STOCK STATUS AND TRENDS  

Ricker $\alpha$ was estimated at `r alpha.CI[2]`(`r alpha.CI[1]`-`r alpha.CI[3]`), $\beta$ at `r beta.CI[2]`(`r beta.CI[1]`-`r beta.CI[3]`), and bias corrected $\sigma$ at  `r `sigma.CI[2]`(`r sigma.CI[1]`-`r sigma.CI[3]`; medians and 95th percentiles (Table *\@ref(tab:tab-prior-posterior) - to make*). 
...
Total recruits are currently at `r filter(data, year==2023)$spawn + filter(data, year==2023)$harvest`, which is just slightly higher than 80% $R_{S{MSY}}$.  
...
The naive PA approach (***naming?***) is intermediate between the 2 and has comparable catch stability to the alternative TAM HCR (difference: -7%), and as an artifact of stability is approximately 4% more likely to have a "good catch year" when looking at the catch index metric. 
...
The only change in rank is the PA HCR is 38% more stable than the alternative TAM HCR. 
...
Catch stability in the sensitivity check was the lowest of all simulations, which isn't surprising when remembering there are only 11 data points between 1993 and present data.  
