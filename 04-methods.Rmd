...
Changes in productivity within the 66 year (i.e 33 odd-year generations) time series could have arose from processes affecting survival outlined in the ecosystem and climate section above. 
Time-varying models can have issues with "chasing noise" and incorrectly estimating benchmarks, but are useful when trying to describe current dynamics (**Holt et. al. in prep - will cite later?**). 
We used a very similar model as used to estimate benchmarks, but removed the AR1 recruitment residuals in equations \@ref(eq:AR1-ricker) and \@ref(eq:AR1), since they are accounting for the same time-varying process described in the new model, but in a different way. In the time-varying Ricker model used here, the $\alpha$ parameter evolves through time as a random walk, yielding annual estimates of productivity: 
