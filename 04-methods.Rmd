...
Escapement and catch data from 1957 to present were provided by the Pacific Salmon Commission (available at @pacificsalmoncommissionFraserRiverPanel2024). 
...
Estimates of total Pink salmon catch include harvest in fisheries occurring in both Canadian and U.S. waters.  Since 1987, PSC staff have applied genetic stock ID from samples taken from test or directed fisheries to estimate total catch of Pink salmon where appropriate. Prior to 1987, Fraser stocks were identified using run reconstruction methods [@whiteGeneticStockIdentification1996].The total return (or recruitment) in any given year was assumed to be the sum of catch and escapement.  
...
The process model is intended to represent the true population dynamics (i.e., free of measurement error and describing true variation in the environment).
...
***Worth noting how Smax prior is N(.75max(spawners)) here? Bit funky with all the transformations***
...
As an upper biological benchmark we used 80% of $S_{MSY}$ following @holtEvaluationBenchmarksConservation2009 and @dfoSustainableFisheriesFramework2022.  
...
\begin{equation}
S_{MSY} = S_{gen}e^\alpha(1-S_{gen}/\beta)
\label{eq:get-Sgen}
\end{equation}

which were used as biological benchmarks to inform HCRs and evaluate their performance.  

Equilibrium spawner abundance, where recruitment exactly replaces spawners, was estimated as:  
...
The reference points above are germane to median yield and recruitment, but because of the assumption that recruitment is lognormally distributed this differs from expected (i.e. mean) yield and recruitment. Therefore, to provide inference about expected yield and recruitment we applied a lognormal bias correction by substituting $\alpha'$ for $\alpha$ in equations \@ref(eq:get-Smsy)-\@ref(eq:get-Seq), where:
...
Fishery objectives included maximizing average harvest and interannual stability in harvest, and minimizing the probability harvests fall below a minimum catch index level which, for illustrative purposes, was chosen as the mean catch of the highest three catches since the year 2000.  
