...
Changes in productivity within the 66 year time series could have arose from processes outlined in the ecosystem and climate section above. 
Time-varying models can have issues with "chasing noise" and incorrectly estimating benchmarks, but are useful when trying to describe current dynamics (**cite Holt et al in prep?**). 
We used a very similar model as used to estimate benchmarks, but removed the AR1 recruitment residuals in equations \@ref(eq:AR1-ricker) and \@ref(eq:AR1), then allowed the Ricker $\alpha$ parameter to evolve as a random walk, yielding annual estimates of productivity: 
...
We simulated future stock trajectories by iterating the process model forward in time for five Pink salmon generations (10 years), thereby generating a posterior predictive distribution of future states, conditioned on the historical data. 
