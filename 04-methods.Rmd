...
Escapement and catch data was provided by colleagues at the Pacific Salmon Commission (*or say internal??*). 
We used the description of escapement estimation methods in @grantFraserRiverPink2014 and knowledge from local area staff to assume relative accuracy among escapement estimation method stanzas; no evidence suggested estimates should be systematically biased low or high, but we assumed there was variable accuracy among methods (Table \@ref(tab:tab-spawner-est-methods)). 
...
The first year of recruitment was not linked to observations of spawner abundance in the spawner-recruitment relationship (Eqn. \@ref(eq:ricker)) and were modeled as random draws from a log-normal distribution with mean $\ln(R_0)$ and variance $\sigma_{R}^2$.
...
Harvest in a given year ($H_y$) was modeled as the product of total run size and the harvest rate ($U_y$) experienced that year test:
...
It is this spawner abundance $S_y$ that is used in Eqn. \@ref(eq:ricker) to produce the recruitment abundance produced by spawning in brood year $y$.


### OBSERVATION MODEL  

A state-space approach was particularly useful for this data set since we expected observation error in escapement to vary among sampling regimes (Table \@ref(tab:tab-spawner-est-methods)). 
Observed spawner abundance was therefore assumed to be log-normally distributed with the CVs converted to log-normal variance following [@forbes_statistical_2011]:  
...
We assumed that harvest had a 5% CV and so harvest observations were log-normally distributed with the CV converted to log-normal variance as per Eqn. \@ref(eq:get-sigma).  
...
\begin{equation}
S_{MSY} = 1 - W(e^1-\alpha)/\beta
\label{eq:get-Smsy}
\end{equation}

where $W$ is the Lambert function [@scheuerellExplicitSolutionCalculating2016], and $\alpha$ and $b$ are intrinsic productivity and the magnitude of within stock density dependence, respectively. 
...
\begin{equation}
S_{MSY} = S_{gen} * exp(\alpha * (1-S_{gen}/\beta))
\label{eq:get-Sgen}
\end{equation}
...
The reference points above are germane to median yield and recruitment, but because of the assumption that recruitment is lognormally distributed this differs from expected (i.e. mean) yield and recruitment. Therefore, to provide inference about expected yield and recruitment we applied a lognormal bias correction by substituting $\alpha'$ for $\alpha$ in equations \@ref(eq:get-Smsy)-\@ref(eq:get-Sgen), where:
...
Closed loop forward simulations used the existing state-space spawner-recruit model (Equation \@ref(eq:ricker)) and projected population dynamics 5 generations forward (10 years). 
...
Simulations began by taking one joint draw of the posterior of parameters and and the most recent year's (i.e. `r max(data$year)`) hidden states of recruits, catch, spawners, and harvest rate. 
...
When looking at target escapement (Figure \@ref(fig:fig-HCRs)) there is a slight hump where target escapement ascends from 0 to approximately 5 million spawners, but then slightly descends to $R_{S_{MSY}}$. 
...
When the recruits are below $S_{gen}$ the ER is 0%; when recruits are between $S_{gen}$ and the upper OCP escapement is fixed at $S_{gen}$, and when recruits are greater than the upper OCP ER is fixed at $U_{MSY}$ (Figure \@ref(fig:fig-HCRs)). 
...
Outcome uncertainty was assumed to be normally distributed with a CV of `r OU.CV*100`%.  


### PERFORMANCE MEASURES  

Simulations were evaluated for each HCR across five metrics that encompass aspects of conservation risk and fishery objectives. 
...
Fishery objectives were addressed by calculating 3) median annual catch, 4) catch stability (1/CV), and 5) a relative catch index; or the percentage of years that fell below an ad hoc threshold, defined as the mean catch of the highest three catches since the year 2000 (Table \@ref(tab:tab-perf-metrics-descriptions)). 
When calculating conservation objectives, we calculated benchmarks at the beginning of a given simulation in order to reflect underlying uncertainty in benchmark estimates; but HCRs did not change within a simulation.  


### ROBUSTNESS TEST AND SENSITIVITY ANALYSIS  

To address uncertainty in estimation of productivity, and to provide resilience to changing productivity forward in the simulation, we also conducted a robustness test that tested a low productivity scenario, where each HCR was tested in a low productivity scenario. 
Initial baseline tests sampled the full posterior, where robustness test sampled the lower tenth percentile of joint draws from the posterior of the productivity parameter (Ricker $\alpha$). 

Some analyses on Fraser Pinks have been cautious to use older data that enumerated escapement with different methods (e.g. @pestalStateSalmonRapid2023, ***others??***). 
To address this, we truncated the data to only be 1993 (a cutoff used above) to present and refit the model, then used population dynamics estimated from that data in the forward simulation, under the same harvest control rules used in the other simulations.  
